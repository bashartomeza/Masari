...
- Forward-only migration 17 adds versioned `CanonicalTripManifest`, ordered membership, per-demand attempts, aggregate offer/reservation ownership, and shared Trip provenance. Migrations 1–16 and frozen tags remain unchanged; existing single-demand canonical and legacy rows are not backfilled or reinterpreted.
...
- Terminal outcomes are categorical and historical. Per-demand attempts exclude prior DriverRoutes; released demands can regroup independently into a later manifest until the five-attempt bound.
...
- Validation evidence: migration 17 applies from empty MySQL 8.0.46, repeat deployment is a no-op, and status is current. A populated migration-16 database created by the M7C3A harness was checksum-backed, restored, upgraded through migration 17, re-deployed as a no-op, and removed; a post-M7C3C1 checksum-backed restore also passed and was removed.
- The M7C3C1 harness passes 118 persistent-state assertions, including ten concurrent accepts, whole-manifest reject/expiry/drift handling, exact capacity restoration, regrouping, legacy isolation, and route-pause invalidation. M7C3A remains 98/98 and M7C1 remains 79/79. Trusted-session, onboarding, public-onboarding 76/76, and route-lifecycle disposable MySQL harnesses pass.
