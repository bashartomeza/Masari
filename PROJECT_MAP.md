...
- The separate keyless `OSM_SELF_HOSTED_VALHALLA` run used the current checksum-verified Geofabrik Israel/Palestine extract and digest-pinned Valhalla 3.8.3. All four required corridor routes succeeded and passed human continuity/plausibility review; warm p95 was 11.291 ms and cold-request p95 121.952 ms. Classification is `PASS_FOR_TESTED_CORRIDOR_ONLY`.
- A disposable localhost Nominatim 5.3.2 import of the same extract succeeded and passed database checks, but strict bilingual fixture acceptance was only 4/8 (Arabic 2/4, English 2/4). `SAMPLE_SIZE_INSUFFICIENT` remains for regional claims and geocoding quality fails the initial target.
...
- Official-source storage, attribution, privacy, commercial, and quota research is recorded in focused M7D1B documents. Google and HERE do not fit the proposed shared multi-year canonical record under reviewed public standard terms; Mapbox and Stadia retain unresolved route-result persistence fields. The OSM path is ODbL and canonical routing-output storage remains `CONDITIONAL / LEGAL_REVIEW_REQUIRED` with attribution mandatory.
- `OSM_VALHALLA_CANDIDATE=CONDITIONAL`; `PROVIDER_SELECTION=NO_PROVIDER_APPROVED_YET`; M7D1B remains `ACTIVE / BLOCKED_ON_REMAINING_PROVIDER_EVIDENCE` pending hosted-provider evidence or an approved component combination, adequate geocoding, broader routes, storage/attribution/legal review, production operations/TCO, and independent approval.
