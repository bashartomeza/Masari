...
- `VALHALLA_ROUTING_CANDIDATE=CONDITIONAL`; `ROUTING_RECOMMENDATION_CANDIDATE=OSM_VALHALLA`; every tested geocoder/search product is `FAIL`; `GEOCODING_QUALITY_LEADER=NOMINATIM`; `SEARCH_QUALITY_LEADER=NOMINATIM`; `GEOCODING_RECOMMENDATION_CANDIDATE=NONE`; `PROVIDER_SELECTION=NO_PROVIDER_APPROVED_YET`. Google Places content storage is `RESTRICTED`; Place ID storage is `PERMITTED_WITH_REFRESH_POLICY`, without production approval. M7D1B remains active on route-storage legal review, production access/operations/TCO, renderer attribution design and final independent approval.
