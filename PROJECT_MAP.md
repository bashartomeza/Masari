...
- Categorical checks found no Mapbox, Google, HERE, or Stadia credential in approved local or CI-secret mechanisms. Those four hosted executions remain `NOT_EXECUTED`; their live geocode, Arabic, route, human-review, and latency sample counts are zero.
...
- `VALHALLA_ROUTING_CANDIDATE=CONDITIONAL`; `NOMINATIM_GEOCODING_CANDIDATE=FAIL`; `PROVIDER_SELECTION=NO_PROVIDER_APPROVED_YET`; M7D1B remains blocked on a separate geocoder comparison, route-storage legal review, production access/operations/TCO, renderer attribution design and final independent approval.
