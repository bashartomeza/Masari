...
- Two independently empty MySQL 8.0.46 databases using `utf8mb4_0900_ai_ci` applied all thirteen migrations. Status was current and repeat deploy was a no-op. The expanded M7C1 suite passes 76 persistent-state assertions, including direct SQL mode/route/demand ownership failures, exact update-versus-hold/release/expiry totals and audit counts, cancellation safety, status/role revocation, stop retirement serialization, ten-way replay atomicity, atomic 50/51-parcel boundaries, and poison-row quarantine.
