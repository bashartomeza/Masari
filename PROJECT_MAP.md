...
[M7C3C1_BACKEND_CANONICAL_SHARED_TRIPS_ACTIVE]
- M7C3C1 is active on feature branch `m7c3c1/backend-shared-trip-aggregation` from approved base `c88378874c1e264465bb207a09a2c2db8b9ed86b`. It is backend-only; M7C3C2 Flutter aggregate presentation, M7D maps/GPS, and M7E realtime/pricing/production dispatch are not started.
- Forward-only migration 17 adds versioned `CanonicalTripManifest`, ordered membership, per-demand attempts, aggregate offer/reservation ownership, and shared Trip provenance. Independent review adds forward-only migration 18, `20260729120000_harden_canonical_shared_trip_integrity`, for one-offer-per-manifest uniqueness, exact offer/reservation/manifest composite ownership, immutable aggregate/member snapshots, and monotonic lifecycle guards. Migrations 1–17 and frozen tags remain unchanged; existing single-demand canonical and legacy rows are not backfilled or reinterpreted.
- A manifest may contain one or more passengers, merchant orders, or both. Multiple dispatches share one frozen manifest, aggregate offer, aggregate reservation, and—after all-or-nothing acceptance—one canonical Trip. One Trip per one-off DriverRoute availability remains enforced.
- `canonical_global_capacity_v1` sums all passenger seats and parcel units for the entire route. One hold and decrement occur at offer time; acceptance confirms without a second decrement; rejection, expiry, and system invalidation restore once. Segment-level capacity reuse is deferred.
- Stable member order, SHA-256 member/manifest fingerprints, exact route-version/departure checks, demand/parcel drift detection, and lock-ordered acceptance prevent partial or stale assignment. Drift dissolves the whole manifest and creates no partial Trip.
- Terminal outcomes are categorical and historical. Per-demand attempts exclude prior DriverRoutes across both single and shared matching modes; released demands can regroup independently into a later manifest until the five-attempt bound.
- Driver APIs are owner-only and serialize aggregate counts and public route-stop events without member identities, demand IDs, fingerprints, reservations, phones, parcel descriptions, recipients, or coordinates. Existing passenger/merchant owner status remains co-member blind.
- `CANONICAL_SHARED_TRIPS_ENABLED` defaults false, requires all canonical dependency gates, and is forbidden in staging/production. No public runner, scheduler, Flutter capability, shared UI, map/GPS/realtime dependency, pricing, or dispatch control is added.
- Correction evidence includes all 18 migrations from empty MySQL 8.0.46, repeat no-op deployment, current status, a populated migration-16 upgrade retaining six single-demand offers and two Trips, and the corrected M7C3C1 harness at 141 persistent-state assertions. Added proofs cover generic/shared expiry ownership, two-way single/shared candidate exclusion, database-side snapshot freeze and terminal resurrection rejection, aggregate ownership constraints, and repeated shared-aware demo reset. Checksum-backed backup/restore passes with trigger-preserving dump normalization and isolated restore cleanup.
- Corrected compatibility evidence is green at 201 API tests, 27 admin tests, and 179 Flutter tests. M7C3A remains 98/98; M7C1 remains 79/79; trusted-session, onboarding, public-onboarding 76/76, and route-lifecycle real-MySQL harnesses pass. Standard/security/tooling validation, production-configured admin, production-like APK, and artifact isolation scans pass. The raw production audit still contains only the four documented Prisma CLI moderate transitive findings; no dependency was changed and no force fix was run.
- Legacy deterministic smoke passes after the hardening trigger’s null-safe compatibility correction: preflight/smoke metrics remain score `0.9317`, sequence `2`, trips `1` versus `6`, distance `21.53` versus `129.19`, cost `43.06` versus `258.38`, winner `masari`. Exact corrected-head PR CI remains required before the independent review can close.
- Local regression passes 200 API tests, 27 admin tests, and 179 Flutter tests with clean typecheck, build, formatting, and analysis. Standard/security/tooling validation and workflow/secret policy pass. The raw production audit contains only the four documented Prisma CLI moderate transitive findings; no force fix was run.
- Production-configured admin and production-like release APK builds pass artifact isolation; the Flutter capability remains unchanged and contains no shared aggregate UI. Demo preflight passes 22/22 and deterministic smoke remains score `0.9317`, sequence `2`, trips `1` versus `6`, distance `21.53` versus `129.19`, cost `43.06` versus `258.38`, winner `masari`. Exact-head PR CI remains required before independent review.
