...
[M7D1_PROVIDER_ABSTRACTION_ROUTE_GEOMETRY_ACTIVE]
- M7D1 is active on `m7d1/provider-abstraction-route-geometry` from approved baseline `abfaf0c943b2ea7e4065f967ff2e07caada40370`. It is not closed or approved for merge. M7D2 and M7E have not begun.
- The backend now has provider-neutral geocode and route contracts, fixed-host Mapbox/Google/HERE/Stadia adapters, a deterministic fake, normalized failures, a single overall deadline with bounded retry, redirect rejection, a size-capped response body, an in-memory coalescing draft-preview cache, a provider circuit breaker, decoded-geometry validation, SHA-256 input/geometry integrity, and a fail-closed kill switch. No provider SDK or new dependency was added.
- Preview and geocode endpoints are admin-only and reference an existing editable draft route version and its server-owned stops. They accept no arbitrary coordinate array, provider URL, hostname, callback, or credential. Preview results are ephemeral and never enter matching, demand, offers, reservations, Trips, tracking, or the deterministic demo.
- Existing migration count remains 18. M7D1 deliberately defers persistence because the current route-version fields cannot represent the complete approved provenance/licensing record and no candidate has passed storage-rights review. Published geometry remains immutable by design; any future correction requires a new route version.
- The committed public Hebron/PPU/Bab Al-Zawiya/Bethlehem Arabic/English bakeoff fixture contains no user or private address data. Fake-provider architecture execution passes 10/10 samples and 100% fixture geocoding. Live Mapbox, Google, HERE, and Stadia executions are `NOT_EXECUTED` because no credentials are present; Palestine route validity and live p95 are not claimed.
- Official terms evidence leaves permanent canonical-geometry storage unresolved for every candidate under the reviewed standard terms/plans. The M7D1 recommendation is `NO_PROVIDER_APPROVED_YET`; Mapbox remains only the conditional bakeoff lead. Production configuration remains `ROUTE_MAPS_ENABLED=false` and `ROUTE_PROVIDER=disabled`.
- No Flutter/Admin map renderer, client token, map/GPS/location permission, foreground/background location service, current-location control, live marker, realtime transport, tracking, navigation, ETA, M7E, pricing, payment, or dispatch control was added.
- Current validation passes 271 API, 54 Admin, and 241 Flutter tests; M7C3C1 145, M7C3A 98, M7C1 79, M7H1 reset 45, legacy online 21, passenger association 12, public onboarding 76, trusted sessions, onboarding, route lifecycle, backup/restore, all five triggers, production Admin/APK scans, and Android packaged permissions. Preflight is 22/22 and smoke remains score 0.9317, sequence 2, trips 1/6, distance 21.53/129.19, cost 43.06/258.38, winner Masari. Exact-head CI is still pending.
