...
- Actor-private trust publication is now keyed by and rechecks the current actor before publishing values or errors. Permanent deterministic logout, logout-all, and terminal-session tests hold Driver A's value `86` or error behind a Completer, authenticate Driver B with `22`, and prove A cannot repopulate B's provider. Auth teardown still preserves the global unresolved secure-operation bundle.
- The real-MySQL reset matrix permanently covers six gate combinations, malformed booleans, staging/production fail-closed behavior, repeatability, pre-existing canonical dispatch cleanup, restrictive foreign keys, and the existing shared manifest/Trip cleanup path. It passes 45 assertions; the shared harness now retains 145 assertions and finishes with exactly two legacy plus two fully gated canonical availability fixtures and no canonical demand/runtime rows.
- Passenger legacy Trips now carry the exact safe `passenger_request_id` into Flutter and are indexed per request; the UI no longer chooses the first/newest same-owner Trip. Focused API/Flutter tests and a repeatable 12-assertion MySQL scenario cover A/B reverse ordering, unassigned/cancelled requests, and cross-owner concealment. Existing M7C3A single-demand and M7C3C1 shared dispatch pointers remain the canonical exact provenance and co-member-blind owner contract.
- `PUT /api/v1/driver/online-state` accepts strict explicit `{ online, expected_route_id? }`, requires `Idempotency-Key`, revalidates an active verified driver, locks profile then legacy routes, and excludes canonical/shared rows. Actor/operation/payload/scope digests use the existing idempotency table; exact replay returns the original logical result, changed payload conflicts, assigned/on-trip deactivation fails safely, and only real transitions produce one safe audit event.
- Flutter operation `legacy_driver_online_state_v1` refreshes authoritative state, fences rapid taps, writes the actor-bound version-2 bundle before sending, retains the same desired state/key through token refresh and ambiguous response loss, shares the single global unresolved-operation slot, and deletes only after authoritative reconciliation. Secure-save failure sends nothing and another actor cannot inspect or replay the bundle.
- Local validation currently passes 217 API tests, 54 Admin tests, 240 Flutter tests, nine tooling tests, workspace typecheck/build, standard/security validation, production Admin/APK builds and artifact scans, and only the four approved Prisma CLI moderate transitives. Disposable MySQL 8.0.46 passed migrations 1–18 from empty, repeat no-op/current status, five triggers, trusted sessions, onboarding, public onboarding 76, route lifecycle, M7C1 79, M7C3A 98, M7C3C1 145, reset 45, association 12, online recovery 21, and checksum-backed backup/restore.
- Demo preflight passes `22/22`; deterministic smoke remains score `0.9317`, sequence `2`, trips `1` versus `6`, distance `21.53` versus `129.19`, cost `43.06` versus `258.38`, winner `masari`. The API 36 emulator installed the configured demo APK and opened Arabic/RTL by default.
- No Prisma schema, migration, or dependency change was made; all 18 migrations, both frozen tags, and M7C3 single/shared matching, capacity, reservation, offer, manifest, and Trip invariants remain unchanged.
