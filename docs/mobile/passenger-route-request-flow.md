...
After a successful or idempotently replayed response, the result states only that the request was recorded. It explicitly says matching is unavailable, no driver is assigned, and no trip was created. Owner history is not added because M7C1 has no owner read contract and exact create replay is sufficient for response-loss recovery.
