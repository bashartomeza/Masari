...
A refresh token is an opaque credential made from a non-secret UUID identifier, its issued mobile role, and 32 cryptographically random bytes. The role component is authenticated by the stored digest and prevents a credential issued before a role change from minting access for the new role. MySQL stores only an HMAC-SHA-256 digest keyed by `REFRESH_TOKEN_PEPPER`; the raw token is returned only by login or successful rotation. The identifier supports indexed lookup but is not returned in error responses.
...
For each refresh, the API parses the identifier, verifies the HMAC in constant-time-safe comparison, and checks token, issued role, mobile eligibility, session, account, expiry, revocation, and security-version state. A role/client mismatch revokes the session instead of issuing access under a new role. A MySQL transaction then conditionally marks the presented token used, creates one replacement, links the old record to it, updates session activity, and records a safe audit event.
...
Refresh lifetime defaults to 30 days and is bounded to 90 days in staging/production. The session expiry set at login is absolute: rotations are capped to its remaining lifetime and do not silently create a sliding indefinite session. Access JWTs remain short-lived and server-revocable during their lifetime. The existing production global limiter also covers refresh; a dedicated distributed refresh limiter is deferred until traffic/abuse evidence requires it. M6C1B is responsible for Flutter refresh-token storage/rotation, retry coordination, logout cleanup, and expiry UX; none of that client behavior is claimed by M6C1A.
