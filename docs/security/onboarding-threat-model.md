...
| Failed or concurrent resend invalidates working OTP | durable dispatch claim; provider outside transaction; fenced finalization; current pointer changes only after accepted delivery |
| Concurrent double use | conditional MySQL updates, invitation/attempt binding, unique redemption, version-fenced idempotency claims, and real race tests |
