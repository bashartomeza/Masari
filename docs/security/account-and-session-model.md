...
Only `active` users may log in, refresh, or use protected routes. Correct credentials for an inactive account return `account_unavailable` without revealing the stored operational reason. Admin-safe reads may show `account_status`, `status_reason`, `status_updated_at`, and `last_login_at`.
...
All successful logins create one `AuthSession`. An access JWT contains only its user subject, role, session ID, security version, and standard issued/expiry timestamps. Every protected request verifies the JWT and then loads the referenced session and user from MySQL. Access fails immediately when:
