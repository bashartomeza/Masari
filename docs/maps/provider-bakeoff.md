...
| OSM self-hosted Valhalla | EXECUTED_ROUTING | NOT_REQUIRED | NOT_EXECUTED_BY_VALHALLA | separate Nominatim 2/4 strict | 4/4 PASS | PASS (11.291 ms warm p95) |
...
The keyless OSM path is separate from Stadia. Local Valhalla used the four required public route concepts and passed the tested corridor review. Self-hosted Nominatim was also technically provisioned, but strict bilingual acceptance was 4/8; the small sample cannot characterize all Palestine geocoding. Detailed reproducibility and limitations are in [OSM/Valhalla routing evidence](osm-valhalla-palestine-evidence.md), [operations](osm-self-hosted-operational-evaluation.md), and [Nominatim evidence](osm-nominatim-evidence.md).
...
| OSM self-hosted | Individual geocodes may be insubstantial under OSMF guidance; systematic collections/derivative databases differ. | ODbL; route output is plausibly a Produced Work but canonical storage needs legal review. | `© OpenStreetMap contributors` plus license access required. | conditional/legal review |
