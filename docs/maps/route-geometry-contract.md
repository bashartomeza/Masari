...
The input checksum covers provider, route/version identity, ordered stop identity and six-decimal coordinates, profile, locale, and routing options. The geometry SHA-256 additionally covers the input digest, provider calculation version, encoding/precision, geometry bytes, distance, and duration. Canonical recursively code-unit-key-sorted JSON makes identical records hash identically; volatile provider request IDs are excluded. Results are verified against their input before serving or caching.

Polyline5, polyline6, segmented polyline6, and HERE flexible-polyline sections are decoded with bounded point counts before acceptance. Validation rejects truncated or overflowing encodings, unsupported HERE versions or dimensions, invalid coordinates, inconsistent precision, empty sections, and geometry without at least two distinct points.
