...
function serviceMock() {
  return {
    ...
    listDriverOffers: vi
      .fn()
      .mockResolvedValue({ offers: [offer], nextCursor: null }),
        ...
        parcels: [
          {
            id: "parcel_1",
            status: "assigned",
            destination_stop_id: "stop_3",
            destination_label: "must-not-leak",
          },
        ],
        canonical_dispatch: {
          status: "offered",
...
describe("M7C3A canonical matching APIs", () => {
  beforeEach(() => {
    ...
    prismaMock.authSession.findUnique.mockImplementation(
      ({ where }: { where: { id: string } }) => {
        ...
        const role = id.startsWith("driver")
          ? "driver"
          : id.startsWith("merchant")
            ? "merchant"
            : "passenger";
        return {
          id: where.id,
  ...
  it("hides offer mutation while keeping owner assignment reads available when matching is disabled", async () => {
    const app = createApp(disabled, {
      canonicalMatchingService: serviceMock() as never,
    });
    await request(app)
      .get("/api/v1/driver/canonical-match-offers")
      .set(auth("driver_1", "driver"))
      .expect(404);
    await request(app)
      .get("/api/v1/passenger/route-requests")
      .set(auth("passenger_1", "passenger"))
      .expect(200);
    await request(app)
      .get("/api/v1/merchant/route-orders")
      .set(auth("merchant_1", "merchant"))
      .expect(200);
  });

  it("requires authentication, the driver role, and verified-driver eligibility", async () => {
    const service = serviceMock();
    const app = createApp(enabled, {
      canonicalMatchingService: service as never,
    });
    ...
    expect(service.listDriverOffers).toHaveBeenCalledWith("driver_1", {
      limit: 25,
    });
  });
  ...
  it("paginates by created_at and id without duplicates when newer offers arrive", async () => {
    ...
    expect(findMany.mock.calls[0]?.[0].where.driver_route.driver.user_id).toBe(
      "driver_1",
    );
  });
  ...
  it("returns bounded driver summaries without score, reservation, phone, or profile data", async () => {
    ...
    expect(response.body.offer).toEqual(
      expect.objectContaining({ id: "offer_1", demand_type: "passenger" }),
    );
  ...
  it("requires idempotency and forwards exact accept/reject scopes", async () => {
    ...
    expect(service.accept).toHaveBeenCalledWith(
      "driver_1",
      "offer_1",
      expect.objectContaining({ idempotencyKey: "accept-key-1" }),
    );
    expect(service.reject).toHaveBeenCalledWith(
      "driver_1",
      "offer_1",
      "schedule_conflict",
      expect.objectContaining({ idempotencyKey: "reject-key-1" }),
    );
  });
  ...
  it("returns owner assignment status without candidate, phone, or parcel-description disclosure", async () => {
    ...
    const passenger = await request(app)
      .get("/api/v1/passenger/route-requests/request_1")
      .set(auth("passenger_1", "passenger"))
      .expect(200);
    const merchant = await request(app)
      .get("/api/v1/merchant/route-orders/order_1")
      .set(auth("merchant_1", "merchant"))
      .expect(200);
    expect(passenger.body.request).toEqual(
      expect.objectContaining({
        assigned: true,
        trip: { id: "trip_1", status: "accepted", vehicle_type: "sedan" },
      }),
    );
    expect(merchant.body.order).toEqual(
      expect.objectContaining({ offer_pending: true, assigned: false }),
    );
  ...
  it("associates each passenger request only with its exact single or shared assigned Trip", () => {
    ...
    const shared = canonicalMatchingSerializers.statusResponse(
      resource(
        "request_shared",
        trip("trip_shared", "canonical_shared_trip_v1"),
      ),
      true,
    );
    const unassigned = canonicalMatchingSerializers.statusResponse(
      resource("request_unassigned", null),
    );
    ...
    expect(
      JSON.stringify({ requestA, requestB, shared, unassigned }),
    ).not.toMatch(
      /must-not-leak|phone|manifest|member|reservation|fingerprint/i,
    );
  });
  ...
  it("scores deterministically and applies documented component weights", () => {
    ...
    expect(canonicalScoring.scoreCandidate(input)).toEqual(
      canonicalScoring.scoreCandidate(input),
    );
    expect(canonicalScoring.scoreCandidate(input)).toEqual({
      score: 0.6725,
  ...
  it("rejects shared and unknown versions in the single-offer serializer", () => {
    expect(() =>
      canonicalMatchingSerializers.offerResponse({
        ...offer,
  ...
  it("does not expose database integrity keys through driver offer serializers", () => {
    const encoded = JSON.stringify(
      canonicalMatchingSerializers.offerResponse(offer),
    );
    for (const field of [
      "demand_checksum",
  ...
  it("keeps the internal runner unavailable in production-like configuration", async () => {
    const production = createConfig({
      APP_ENV: "production",
      DATABASE_URL:
        "mysql://database-user:database-password@db.internal:3306/masari",
      JWT_SECRET: "production-jwt-secret-with-more-than-thirty-two-characters",
      REFRESH_TOKEN_PEPPER:
        "production-refresh-secret-with-more-than-thirty-two-characters",
