...
const users: Record<
  string,
  { id: string; role: Role; name: string; phone: string; demo_account: boolean }
> = {
  driver_1: {
    id: "driver_1",
    role: "driver",
...
const baseMatch = {
  ...
  driver_route: {
    id: "route_1",
    driver_id: "profile_1",
    driver: { id: "profile_1", user_id: "driver_1" },
  },
...
describe("trip acceptance, status, and tracking", () => {
  beforeEach(() => {
    ...
    prismaMock.user.findUnique.mockImplementation(
      ({ where }: { where: { id?: string } }) => {
        ...
        return user
          ? { ...user, account_status: "active", security_version: 1 }
          : null;
      },
    );
  ...
  it("driver cannot accept another driver route match", async () => {
    prismaMock.match.findUnique.mockResolvedValue({
      ...baseMatch,
      driver_route: {
        ...baseMatch.driver_route,
        driver: { user_id: "driver_2" },
      },
    });
  ...
  it("preserves exact passenger request provenance for same-owner Trip lists regardless of ordering", async () => {
    ...
    const first = await request(createApp())
      ...
      .expect(200);
    expect(
      first.body.trips.map(
        (trip: { id: string; passenger_request_id: string }) => [
          trip.id,
    ...
    const reversed = await request(createApp())
      ...
      .expect(200);
    expect(
      reversed.body.trips.find((trip: { id: string }) => trip.id === "trip_a"),
    ).toHaveProperty("passenger_request_id", "request_a");
    expect(
      reversed.body.trips.find((trip: { id: string }) => trip.id === "trip_b"),
  ...
  it("valid status sequence works", async () => {
    ...
    prismaMock.trip.findUnique.mockImplementation(() =>
      baseTrip(statuses.shift() ?? "completed"),
    );
    prismaMock.trip.update.mockImplementation(({ data }) => ({
      ...baseTrip(data.status),
      status: data.status,
    }));
  ...
  it("picked_up updates related request, batch, and parcels", async () => {
    ...
    expect(prismaMock.parcelBatch.update).toHaveBeenCalledWith(
      expect.objectContaining({ data: { status: "picked_up" } }),
    );
    expect(prismaMock.parcel.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({ data: { status: "picked_up" } }),
    );
  });
  ...
  it("simulate step creates deterministic location events", async () => {
    prismaMock.trip.findUnique.mockResolvedValue(baseTrip());
    prismaMock.locationEvent.findFirst
      .mockResolvedValueOnce(null)
      .mockResolvedValueOnce({ sequence: 0 });
    prismaMock.locationEvent.create.mockImplementation(({ data }) => ({
      id: `loc_${data.sequence}`,
      ...data,
    }));

    const first = await request(createApp())
      .post("/api/v1/trips/trip_1/simulate/step")
      .set(auth("driver_1"))
      .expect(201);
  ...
  it("latest location endpoint returns latest event", async () => {
    prismaMock.trip.findUnique.mockResolvedValue(baseTrip());
    prismaMock.locationEvent.findFirst.mockResolvedValue({
      id: "loc_2",
      sequence: 2,
      lat: "31.585000",
      lng: "35.123000",
    ...
    const response = await request(createApp())
      .get("/api/v1/trips/trip_1/location")
      .set(auth("passenger_1"))
      .expect(200);
